...
router.get("/", (req, res) => {
  ...
  const fetchArticles = async () => {
    ...
    // Fetch
    const autoblogRaw = await parser.parseURL(
      "https://www.autoblog.com/category/green-auto-news/rss.xml"
    );
    const autoevolutionRaw = await parser.parseURL(
      "https://www.autoevolution.com/rss/backend-green.xml"
    );
    const insideevsRaw = await parser.parseURL(
    ...
    import("node-fetch")
      .then(async (module) => {
        ...
        // Continue with your code that uses fetch
